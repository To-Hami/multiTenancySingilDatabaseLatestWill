                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            ...
                            @if(auth()->user()->tenants()->count() > 1)
                                @foreach(auth()->user()->tenants as $tenant)
                                    ...
                                    <a href="{{route('change.tenant',$tenant->id)}}"
                                       class="@if (auth()->user()->current_tenant_id == $tenant->id) font-bold border-gray-100  @endif  px-4 py-2 flex items-center text-sm font-medium text-gray-500
                                                   hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700
                                                   focus:border-gray-300 transition duration-150 ease-in-out">
                                        {{$tenant->name}}
                                    </a>
